...
class Book(models.Model): # book_book 테이블
    ...
    author = models.CharField(
        ...
        validators=[
            ...
            # MinLengthValidator(2, "2글자 이상 입력하세요.") # 위와 같음
        ]
    )
    ...
    ip = models.GenericIPAddressField( # IP 자동 입력
        blank=True,
        null=True,
    )
    publication_date = models.DateTimeField(
        auto_now_add=True, # 작성 일시 자동 입력
    )
    ...
    class Meta:
        ...
        unique_together = [('title', 'author')] # 책 제목과 저자명이 모두 일치하는 item 저장 불가