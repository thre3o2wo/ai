...
# 2. 제어문 : {% %}
    # 2-1. 조건문 : {% if 조건 %} A태그 {% elif 조건 %} B태그 {% else %} C태그 {% endif %}
    # 2-2. 반복문
        # {% for var in vars %}
        #   <태그> {{loop.index}}. {{var}}</태그> # 1. 1행 변수
        #       loop.index : 1부터 순번 / loop.first: 첫 번째 loop인지 여부 / loop.last : 마지막 loop인지 여부
        # {% endfor %}
# 3. 헤더나 푸터 {% include "header.html" %} {% extends "base.html" %}
