# 파일명 app.py일 경우 실행 : flask run --port 8090 -- debug
...
# 터미널에서 flask run --debug

@app.route("/join", methods=["GET", "POST"])
def join():
    print(request.method) # 어떤 방식으로 들어왔는지
    if request.method == 'GET':
        return render_template("2_crud/join.html")
    elif request.method == 'POST':
        # print(request.form) # POST 방식으로 전달된 파라미터 내용
