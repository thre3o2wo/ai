...
# cmd - ctrl+j
# 실행 - streamlit run 6_assistantWebBot.py
# 종료 - ctrl+c 
...
def main():
    ...
    # client 생성
    client = OpenAI()
    # assistant와 thred 초기화(각 id 세션에 추가)
    ...
    if st.button('대화 이력 백업'):
        ...
        if messages.data:
            # messages가 빈 리스트가 아니면, 시간 순서대로 정렬한 후 파일 백업
            ...
            with open('data/ch7_history.txt', 'w', encoding='utf-8') as f:
                for msg in sorted_messages:
                    ...
                    f.write(row)
                st.info('대화 내역을 저장하였습니다.')
        else:
            st.warning('저장할 대화 내역이 없습니다.')
